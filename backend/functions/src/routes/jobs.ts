...
router.get("/", async (req, res) => {
    try {
        ...
        if (location) apiUrl.searchParams.set("location", String(location));
        if (sort_by) apiUrl.searchParams.set("sort_by", String(sort_by));
        if (page) apiUrl.searchParams.set("page", String(page));
